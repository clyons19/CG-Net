...
# Contains:
# * Loss Functions for I=image estimation neural networks
...
class MSE_loss: # Mean Squared Error
    def __init__(self, power = 1, normalize = False):
        self.power = power    # power applied to each norm (e.g. power = 1 gives MSE, power = 1/2 gives root MSE)
...
class MAE_loss: # Mean Absolute Error
    def __init__(self, power = 1, normalize = False):
        self.power = power    # power applied to each norm (e.g. power = 1 gives MAE, power = 1/2 gives root MAE)
...
class SSIM_loss: # Structural Similarity Index
    def __init__(self, Phi, img_size, scale):
        self.Phi = tf.constant(Phi, dtype = float)  # Representation dictionary of the images
...
class weighted_SSIM_loss: # Weighted loss between SSIM and another loss
